...
resource "aws_lambda_permission" "short_url_lambda_permssion_short_url_list" {
  ...
  source_arn    = "arn:aws:execute-api:${var.aws_region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.short_urls_api_gateway.id}/*/${aws_api_gateway_method.short_url_api_get.http_method}${aws_api_gateway_resource.short_url_api_resource_admin.path}"
}
